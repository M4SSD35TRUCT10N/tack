...
int main(void) {
  ...
  ensure_dir_recursive("build/tests/tap-pass-repo/src");
  ensure_dir_recursive("build/tests/tap-pass-repo/tests");
  ensure_dir_recursive("build/tests/tap-fail-repo/src");
  ensure_dir_recursive("build/tests/tap-fail-repo/tests");
  ensure_dir_recursive("build/tests/bin");

  if (write_text_file("build/tests/tap-pass-repo/src/main.c",
      "int main(void) { return 0; }\n") != 0) {
    fprintf(stderr, "FAIL write-pass-main\n");
    return 1;
  }
  if (write_text_file("build/tests/tap-pass-repo/tests/pass_test.c",
      "#include <stdio.h>\nint main(void) { puts(\"pass_test: ok\"); return 0; }\n") != 0) {
    fprintf(stderr, "FAIL write-pass-test\n");
    return 1;
  }
  if (write_text_file("build/tests/tap-fail-repo/src/main.c",
      "int main(void) { return 0; }\n") != 0) {
    fprintf(stderr, "FAIL write-fail-main\n");
    return 1;
  }
  if (write_text_file("build/tests/tap-fail-repo/tests/fail_test.c",
      "int main(void) { return 1; }\n") != 0) {
    fprintf(stderr, "FAIL write-fail-test\n");
    return 1;
  }

  path_join(pass_repo, sizeof(pass_repo), repo_root, "build/tests/tap-pass-repo");
  path_join(fail_repo, sizeof(fail_repo), repo_root, "build/tests/tap-fail-repo");
  path_join(tack_bin, sizeof(tack_bin), repo_root, "build/tests/bin/tack_tap");
  path_join(pass_tap, sizeof(pass_tap), repo_root, "build/tests/pass.tap");
  path_join(fail_tap, sizeof(fail_tap), repo_root, "build/tests/fail.tap");
  ...
  if (run_argv_wait_const(cc_argv, 0) != 0) {
    fprintf(stderr, "FAIL build-tack-tap\n");
    return 1;
  }
  ...
  pass_argv[1] = "test";
  pass_argv[2] = "debug";
  pass_argv[3] = "--report-tap";
  pass_argv[4] = pass_tap;
  pass_argv[5] = 0;
  if (run_in_repo(pass_repo, pass_argv) != 0) {
    fprintf(stderr, "FAIL run-pass-tap\n");
    return 1;
  }
  if (load_text_file(pass_tap, buf, sizeof(buf)) != 0) {
    fprintf(stderr, "FAIL read-pass-tap\n");
    return 1;
  }
  if (!contains_text(buf, "1..1")) {
    fprintf(stderr, "FAIL tap-plan\n");
    failures++;
  }
  if (!contains_text(buf, "ok 1 - pass_test.c")) {
    fprintf(stderr, "FAIL tap-ok-line\n");
    failures++;
  }
  if (!contains_text(buf, "# compiled: true")) {
    fprintf(stderr, "FAIL tap-compiled-line\n");
    failures++;
  }
  ...
  fail_argv[1] = "test";
  fail_argv[2] = "debug";
  fail_argv[3] = "--report-tap";
  fail_argv[4] = fail_tap;
  fail_argv[5] = 0;
  if (run_in_repo(fail_repo, fail_argv) == 0) {
    fprintf(stderr, "FAIL fail-tap-expected-error\n");
    return 1;
  }
  if (load_text_file(fail_tap, buf, sizeof(buf)) != 0) {
    fprintf(stderr, "FAIL read-fail-tap\n");
    return 1;
  }
  if (!contains_text(buf, "not ok 1 - fail_test.c")) {
    fprintf(stderr, "FAIL tap-not-ok-line\n");
    failures++;
  }
  if (!contains_text(buf, "Bail out! tack fail-fast after test failure")) {
    fprintf(stderr, "FAIL tap-bail-out\n");
    failures++;
  }
  ...
  puts("tap_report_output_test: ok");
  return 0;
}
