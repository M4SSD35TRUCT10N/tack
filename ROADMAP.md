...
- 0.8-Baustein jetzt gelegt: `--ci` und ein stabiler `TACK_SUMMARY`-Block als runner-taugliche Basisschicht; die nächste Maschinenebene ist `--events-jsonl`, dokumentiert in [SPEC 0005](docs/specs/0005-events-jsonl-foundation.md).
...
- 0.8 building block now in place: `--ci` and a stable `TACK_SUMMARY` trailer as a runner-friendly base layer; the next machine-facing layer is `--events-jsonl`, documented in [SPEC 0005](docs/specs/0005-events-jsonl-foundation.md).
