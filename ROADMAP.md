...
- 0.8-Baustein jetzt gelegt: `--ci` und ein stabiler `TACK_SUMMARY`-Block als CI-taugliche Basisschicht. Darauf folgen `--events-jsonl` als Ereignisschicht ([SPEC 0005](docs/specs/0005-events-jsonl-foundation.md)) und `--report-tap` als erster Testbericht-Adapter ([SPEC 0006](docs/specs/0006-tap-report-foundation.md)).
...
- 0.8 building block now in place: `--ci` and a stable `TACK_SUMMARY` trailer as a CI-oriented base layer. On top of that come `--events-jsonl` as the event layer ([SPEC 0005](docs/specs/0005-events-jsonl-foundation.md)) and `--report-tap` as the first test-report adapter ([SPEC 0006](docs/specs/0006-tap-report-foundation.md)).
